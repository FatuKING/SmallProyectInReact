...
export function Register () {
  const { email, password, checkPassword, validateEmail, validatePassword, validateCheckPassword, errorEmail, errorPassword, errorCheckPassword, handleRegister } = useAuth()
  const [showPassword, setShowPassword] = useState(false)
  const [showCheckPassword, setShowCheckPassword] = useState(false)
  const idEmail = useId()
  const idPassword = useId()
  const idCheckPassword = useId()
  ...
  return (
    <>
      <form className='h-full w-1/2 flex flex-col gap-3 justify-center items-center' onSubmit={handleRegister}>
        <h1 className='inline-block text-center text-xl font-bold'>Registrarse</h1>

        <label className='group relative' htmlFor={idEmail}>
          <input
            onChange={validateEmail}
            value={email}
            id={idEmail}
            className='bg-transparent p-2 border border-gray-500 w-80 rounded focus:border-blue-700 focus:outline-none'
            name='Email'
            type='text'
        ...
        {errorEmail && <span className='text-sm text-red-500'>{errorEmail}</span>}

        <label className='group relative' htmlFor={idPassword}>
          <input
          ...
          <button onClick={toggleVisibility} className='absolute right-4 bottom-3'>
        ...
        {errorPassword && <span className='text-sm text-red-500'>{errorPassword || null} </span>}

        <label className='group relative' htmlFor={idCheckPassword}>
          <input
            ...
            required
          />
          <div className='absolute -top-3 left-2 bg-black px-1 text-gray-500 text-sm font-bold group-focus-within:text-blue-700'>Repetir Contraseña</div>
          <button onClick={toggleVisibilityCheck} className='absolute right-4 bottom-3'>
            <svg width='22px' height='22px' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg' stroke='#6B7280'><g id='SVGRepo_bgCarrier' strokeWidth='0' /><g id='SVGRepo_tracerCarrier' strokeLinecap='round' strokeLinejoin='round' /><g id='SVGRepo_iconCarrier'> <path fillRule='evenodd' clipRule='evenodd' d='M12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9ZM11 12C11 11.4477 11.4477 11 12 11C12.5523 11 13 11.4477 13 12C13 12.5523 12.5523 13 12 13C11.4477 13 11 12.5523 11 12Z' fill='#6B7280' /> <path fillRule='evenodd' clipRule='evenodd' d='M21.83 11.2807C19.542 7.15186 15.8122 5 12 5C8.18777 5 4.45796 7.15186 2.17003 11.2807C1.94637 11.6844 1.94361 12.1821 2.16029 12.5876C4.41183 16.8013 8.1628 19 12 19C15.8372 19 19.5882 16.8013 21.8397 12.5876C22.0564 12.1821 22.0536 11.6844 21.83 11.2807ZM12 17C9.06097 17 6.04052 15.3724 4.09173 11.9487C6.06862 8.59614 9.07319 7 12 7C14.9268 7 17.9314 8.59614 19.9083 11.9487C17.9595 15.3724 14.939 17 12 17Z' fill='#6B7280' /> </g></svg>
          </button>
        </label>

        {errorCheckPassword && <span className='text-sm text-red-500'>{errorCheckPassword || null} </span>}

        <button
          className='bg-blue-700 p-2 w-80'
          {...(errorCheckPassword !== null || errorEmail !== null || errorPassword !== null ? { disabled: true } : {})}
        >Acceder
        </button>
